from typing import DefaultDict, Tuple, Iterable, Callable
...
class Interpreter:
    ...
    def run(self, input_signal: Iterable[int] = [0],
            process_output: Callable[[int], None] = print):
        '''
        If input_signal should be an iterable
        '''
        mem = self.mem
        ind = self._ind
        input_signal = iter(input_signal)
        ...
        while True:
            ...
            if op_code == 1:
                ...
                if value is Interpreter.STOP_SIGNAL:
                    break
